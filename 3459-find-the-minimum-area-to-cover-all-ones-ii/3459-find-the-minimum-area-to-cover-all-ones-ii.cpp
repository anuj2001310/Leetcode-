...
public:
    ...
    int f1(int si, int ei, int sj, int ej) {
        ...
        while (si < ei && !get_row_count(si, sj, ej))
            si++;
        while (ei > si && !get_row_count(ei, sj, ej))
            ei--;
        while (sj < ej && !get_col_count(sj, si, ei))
            sj++;
        while (ej > sj && !get_col_count(ej, si, ei))
            ej--;
    ...
    int f2(int si, int ei, int sj,
           int ej) { // min area to cover all ones in range with 2 rect;
        int res = max(0, (ei - si + 1)) * max(0, (ej - sj + 1));

        for (int i = si; i <= ei - 1;
             i++) { // row partition, [si, i] U [i + 1, ei];
            int curr = f1(si, i, sj, ej) + f1(i + 1, ei, sj, ej);
            res = min(res, curr);
        }

        for (int j = sj; j <= ej - 1;
             j++) { // col partiion, [sj, j] U [j + 1, ej];
    ...
    int minimumSum(vector<vector<int>>& grid) {
        ...
        row_sfx = vector<vector<int>>(n, vector<int>(m + 1)),
        col_sfx = vector<vector<int>>(m, vector<int>(n + 1));
